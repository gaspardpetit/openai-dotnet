...
{
    public partial class InternalEvalRunList : IJsonModel<InternalEvalRunList>
    {
        internal InternalEvalRunList() : this(null, null, null, null, default, null)
        {
        }
        ...
        InternalEvalRunList IJsonModel<InternalEvalRunList>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options) => JsonModelCreateCore(ref reader, options);

        protected virtual InternalEvalRunList JsonModelCreateCore(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<InternalEvalRunList>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(InternalEvalRunList)} does not support reading '{format}' format.");
            }
            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeInternalEvalRunList(document.RootElement, options);
        }

        internal static InternalEvalRunList DeserializeInternalEvalRunList(JsonElement element, ModelReaderWriterOptions options)
        {
            ...
            IList<InternalEvalRun> data = default;
            ...
            foreach (var prop in element.EnumerateObject())
            {
                ...
                {
                    List<InternalEvalRun> array = new List<InternalEvalRun>();
                    foreach (var item in prop.Value.EnumerateArray())
                    {
                        array.Add(InternalEvalRun.DeserializeInternalEvalRun(item, options));
                    }
                    data = array;
                    continue;
                }
